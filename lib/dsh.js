 ...
 * @param {boolean} [options.captureStdout] 是否捕获 stdout 到返回值（默认不捕获）
 * @param {boolean} [options.forwardStdout] 捕获时是否同时回显到父进程 stdout（默认 false）
 * @param {(chunk: unknown) => void} [options.onStdout] 每段 stdout 的回调（给了就会 pipe）
 * @param {boolean} [options.forwardStderr] 是否把 stderr 实时转发到父进程（默认 true）
 * @param {Record<string,string>} [options.env] 追加/覆盖的环境变量（合并进 process.env）
 * @param {(child: import("node:child_process").ChildProcess) => void} [options.onChild]
 *   拿到子进程句柄的回调（重启时用它终止进程树）
...
export function spawnDsh(args, options = {}) {
  ...
  // 要回显或要回调就必须 pipe；否则保持 inherit，不改变原有输出路径
  const pipeStdout = captureStdout || typeof onStdout === "function";
  return new Promise((resolve) => {
    ...
    try {
      ...
      child = spawn("dsh", args, {
        ...
        stdio: ["inherit", pipeStdout ? "pipe" : "inherit", "pipe"],
        env: env === undefined ? undefined : { ...process.env, ...env },
      });
    } catch (error) {
      // spawn 在某些环境（如受限沙箱的命名管道）会同步抛出而不是 emit('error')，
      // 这里统一成返回值，避免调用方炸掉
    ...
    if (pipeStdout) {
      child.stdout.on("data", (chunk) => {
        ...
        if (forwardStdout) process.stdout.write(chunk);
        onStdout?.(chunk);
      });
    }
